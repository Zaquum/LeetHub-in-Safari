...
LeetCodeV1.prototype.parseQuestion = function () {
  ...
  if (checkElem(questionElem)) {
    ...
    const badge = `<img src='https://img.shields.io/badge/Difficulty-${difficulty}-${badgeColor}' alt='Difficulty: ${difficulty}' />`;

    // Final formatting of the contents of the README for each problem
    const markdown = `<h2><a href="${questionUrl}">${qtitle}</a></h2>${badge}<hr>${qbody}`;
    // Final formatting of the contents of the README for each problem
...
LeetCodeV2.prototype.parseQuestion = function () {
  ...
  if (this.submissionData != null) {
    ...
    const qTitle = `${this.submissionData.question.questionFrontendId}. ${this.submissionData.question.title}`;
    ...
    // Final formatting of the contents of the README for each problem
    markdown = `<h2><a href="${questionUrl}">${qTitle}</a></h2>${badge}<hr>${qBody}</hr>`;

    // // Final formatting of the contents of the README for each problem
    // markdown = `<h2><a href="${questionUrl}">${qTitle}</a></h2>${difficulty}<hr>${qBody}`;
  } else {
